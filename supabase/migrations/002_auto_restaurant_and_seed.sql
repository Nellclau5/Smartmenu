-- Smart Menu — Création auto restaurant + plats de démo

-- Fonction : insère les plats de démonstration
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM menu_items WHERE restaurant_id = p_restaurant_id LIMIT 1
  ) THEN
    RETURN;
  END IF;
...
-- Fonction : crée un restaurant à l'inscription d'un nouvel utilisateur
...
-- Trigger sur création de compte auth
